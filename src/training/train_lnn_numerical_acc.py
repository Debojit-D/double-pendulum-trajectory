...
DATA_DIR = Path(
    "/home/iitgn-robotics/Debojit_WS/double-pendulum-trajectory/data/SampleIdeal1"
)
MANIFEST = DATA_DIR / "double_pendulum_manifest_ideal.json"

OUT_DIR = DATA_DIR / "lnn_model"
...
# Savitzky–Golay parameters for ddq estimation
SAVGOL_WINDOW = 51
SAVGOL_POLYORDER = 3
...
def _estimate_ddq_from_q(q: np.ndarray, t: np.ndarray, win: int, poly: int) -> np.ndarray:
    ...
    ddq = savgol_filter(
        q, window_length=win_robust, polyorder=poly,
...
def _build_dataset(
    ...
    Only keep samples with t <= t_max, and then downsample with `stride`.
    """
    ...
    for p in tqdm(csv_paths, desc="Loading trajectories for LNN"):
        ...
        # Require minimum length for SavGol
        if len(t) < 6:
            continue

        ddq = _estimate_ddq_from_q(q, t, win=SAVGOL_WINDOW, poly=max(3, SAVGOL_POLYORDER))
